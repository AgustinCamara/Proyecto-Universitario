...
void altaPublicacion(){
    ...
    std::vector<DTInmuebleAdministrado> inmueblesAdmin = controladorUsuarios->listarInmueblesAdministrados(nicknameInmobiliaria);
...
tipoInmueble = TipoInmueble::Apartamento;
    ...
    if (verDetalle == 1){
        ...
        DTInmueble det = controladorInmuebles->detalleInmueblePublicacion(codigoPublicacion);
        int codigoInmueble = det.getCodigo();
        ...
        if (apto) {
            DTApartamento dtapto = apto->getDTApartamento();
            DTInmueble dtInm = apto->getDTInmueble();  
        ...
        } else if (Casa* casa = dynamic_cast<Casa*>(inm)) {
            DTCasa dtcasa = casa->getDTCasa();
            DTInmueble dtInm = casa->getDTInmueble(); 
...
void eliminarInmueble(){
    ...
    std::vector<DTInmuebleListado> inmuebles = factory->getControladorInmuebles()->listarInmuebles();
    ...
    std::cin.ignore();
    std::cout << "Detalle del inmueble:\n";
    DTInmueble dtInm = factory->getInstance()->getControladorInmuebles()->detalleInmueble(codigoInmueble);
    IControladorInmuebles* controladorInmuebles = factory->getControladorInmuebles();
...
void suscribirseNotificaciones(){
    ...
    std::string nicknameInmobiliaria;
    std::getline(std::cin, nicknameInmobiliaria);
    bool suscripcionOk = factory->getInstance()->getControladorUsuarios()->suscribirAInmobiliaria(nicknameInmobiliaria);
...
void consultaNotificaciones() {
    ...
    std::string nicknameCliente;
    std::getline(std::cin, nicknameCliente);
    std::set<DTNotificacion> notificaciones = factory->getInstance()->getControladorUsuarios()->listarNotificaciones(nicknameCliente);
    std::set<DTNotificacion>::iterator iter;
   for (iter = notificaciones.begin(); iter != notificaciones.end(); ++iter) {
       std::cout << "- Nickname: " << iter->getNickname() << "- Mensaje:" << iter->getMensaje() << std::endl;
   }
   factory->getInstance()->getControladorUsuarios()->eliminarNotificaciones();
...
void eliminarSuscripciones(){
    ...
    bool bajaOk = factory->getControladorUsuarios()->desuscribirDeInmobiliaria(nicknameInmobiliaria);
